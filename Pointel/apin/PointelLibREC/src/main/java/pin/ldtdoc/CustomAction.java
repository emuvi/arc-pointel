...
public class CustomAction extends StyledEditorKit.StyledTextAction {
    ...
    public CustomAction(LdtDoc lDoc, String actionName, HTML.Tag inTag, Hashtable attribs) {
        super(actionName);
        parentLDoc = lDoc;
        htmlTag = inTag;
        htmlAttribs = attribs;
    ...
    public CustomAction(LdtDoc lDoc, String actionName, HTML.Tag inTag) {
        this(lDoc, actionName, inTag, new Hashtable());
    }
    ...
    public void actionPerformed(ActionEvent ae) {
        if (this.isEnabled()) {
            ...
            if (selText == null || textLength < 1) {
                SimpleInfoDialog sidWarn = new SimpleInfoDialog(null, "Erro", true, "Nenhum Texto Selecionado", SimpleInfoDialog.ERROR).mostra();
            } else {
                ...
                if (htmlTag.toString().equals(HTML.Tag.A.toString())) {
                    if (!htmlAttribs.containsKey("href")) {
                        UserInputAnchorDialog uidInput = new UserInputAnchorDialog(parentLDoc, "Vínculo", true, currentAnchor);
                    ...
                    if (htmlAttribs.containsKey("color")) {
                        Color color = new JColorChooser().showDialog(null, "Cor", Color.black);
                        if (color != null) {
                ...
                if (htmlAttribs2.size() > 0) {
                    ...
                    SimpleAttributeSet baseAttrs = new SimpleAttributeSet(parentLDoc.controle().getCharacterAttributes());
