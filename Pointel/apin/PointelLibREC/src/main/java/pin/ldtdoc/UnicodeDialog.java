...
public class UnicodeDialog extends JDialog implements ActionListener {
 ...
 /*
    ...
    public UnicodeDialog(LdtDoc parent, String title, boolean bModal, int index) {
        super((Frame) null, title, bModal);
        parentLDoc = parent;
        init(index);
    }
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getActionCommand().equals(CMDCHANGEBLOCK)) {
            ...
            try {
                parentLDoc.insertUnicodeChar(ae.getActionCommand());
            } catch (Exception ex) {
                Utl.registra(ex);
            }
        }
    ...
    public void init(int startIndex) {
        ...
        for (int counter = 0; counter < UNICODEBLOCKSIZE; counter++) {
            ...
            if (counter == 0) {
                FontRenderContext frcLocal = ((java.awt.Graphics2D) (parentLDoc.getGraphics())).getFontRenderContext();
        ...
        selectorPanel.add(new JLabel("Bloco"));
        selectorPanel.add(jcmbBlockSelector);
        selectorPanel.add(new JLabel("Página"));
        selectorPanel.add(jcmbPageSelector);

        JPanel buttonPanel = new JPanel();

        JButton closeButton = new JButton("Fecha");
