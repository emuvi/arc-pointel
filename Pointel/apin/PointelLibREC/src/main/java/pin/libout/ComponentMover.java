...
/**
 * This class allows you to move a Component by using a mouse. The Component moved can be a high level Window (ie. Window, Frame, Dialog) in which case the Window is moved within the desktop. Or the Component can belong to a Container in which case the Component is moved within the Container.
 *
 * When moving a Window, the listener can be added to a child Component of the Window. In this case attempting to move the child will result in the Window moving. For example, you might create a custom "Title Bar" for an undecorated Window and moving of the Window is accomplished by moving the title bar only. Multiple components can be registered as "window movers".
 *
 * Components can be registered when the class is created. Additional components can be added at any time using the registerComponent() method.
 */
public class ComponentMover extends MouseAdapter {
     ...
     * @param component the Components to be registered for forwarding drag events to the ancestor Component.
     */
     ...
     * @param components the Components to be registered for forwarding drag events to the parent component to be moved
     */
     ...
     *
     * @param changeCursor when true the cursor will be changed to the Cursor.MOVE_CURSOR while the mouse is pressed
     */
    ...
    /**
     * Set the edge insets. The insets specify how close to each edge of the parent component that the child component can be moved. Positive values means the component must be contained within the parent. Negative values means the component can be moved outside the parent.
     *
     * @param edgeInsets
     */
    ...
    private void deregisterComponent() {
        orign.removeMouseListener(this);
        orign.setBorder(border);
        orign.revalidate();
    }
    ...
    private void registerComponent(JComponent component) {
        ...
        component.setBorder(BorderFactory.createLineBorder(new Color(136, 0, 21), 4, true));
    }
     ...
     */
    public ComponentMover setSnapSize(Dimension snapSize) {
    ...
    private void setupForDragging(MouseEvent e) {
        ...
        {
            destination = (JComponent) SwingUtilities.getAncestorOfClass(destinationClass, source);
        }
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if (orign.getParent() != null) {
            orign.getParent().repaint();
        }
    }
