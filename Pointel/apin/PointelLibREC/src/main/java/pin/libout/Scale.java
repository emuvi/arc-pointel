 ...
 * <p/>
 * This class also implements an optimized version of the incremental scaling algorithm presented by Chris Campbell in his <a href="http://today.java
 * .net/pub/a/today/2007/04/03/perils-of-image-getscaledinstance.html">Perils of Image.getScaledInstance()</a> article in order to give the best-looking image resize results (e.g. generating thumbnails that aren't blurry or jagged).
 * <p>
 ...
 * <strong>ALL</strong> of these pain points for you internally by shuffling all images into one of two types:
 * <ol>
 * <li>{@link BufferedImage#TYPE_INT_RGB}</li>
 * <li>{@link BufferedImage#TYPE_INT_ARGB}</li>
 * </ol>
 * depending on if the source image utilizes transparency or not. This is a recommended approach by the Java2D team for dealing with poorly (or non) supported image types. More can be read about this issue <a href=
 ...
 * <h3>Thread Safety</h3>
 * The {@link Scale} class is <strong>thread-safe</strong> (as all the methods are <code>static</code>); this class maintains no internal state while performing any of the provided operations and is safe to call simultaneously from multiple threads.
 * <h3>Logging</h3>
...
public class Scale {
     ...
     * <h3>Performance</h3>
     * Use of this (and other) {@link ConvolveOp}s are hardware accelerated when possible. For more information on if your image op is hardware accelerated or not, check the source code of the underlying JDK class that actually executes the Op code, <a href=
    ...
    public static enum Method {
         ...
         * <p/>
         * The scaling algorithm will use the {@link Scale#THRESHOLD_QUALITY_BALANCED} or {@link Scale#THRESHOLD_BALANCED_SPEED} thresholds as cut-offs to decide between selecting the <code>QUALITY</code>, <code>BALANCED</code> or <code>SPEED</code> scaling algorithms.
         * <p/>
    ...
    public static enum Mode {
         ...
         *
         * See "Image Proportions" in the {@link Scale} class description for more detail.
         */
        AUTOMATIC,
        /**
     ...
     * href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4965606">a decade-old JDK bug</a> that can cause a {@link RasterFormatException} when applying a perfectly valid {@link BufferedImageOp}s to images.
     * <p/>
     ...
     * <p/>
     * <strong>NOTE</strong>: Because Java will auto-box primitive arguments into Objects when building out the <code>params</code> array, care should be taken not to call this method with primitive values unless {@link Scale#DEBUG} is <code>true</code>; otherwise the VM will be spending time performing unnecessary auto-boxing calculations.
     *
     ...
     *
     * @see <a
     *      href="http://www.mail-archive.com/java2d-interest@capra.eng.sun.com/msg05621.html">How Java2D handles poorly supported image types</a>
     * @see <a
     *      href="http://code.google.com/p/java-image-scaling/source/browse/trunk/src/main/java/com/mortennobel/imagescaling/MultiStepRescaleOp.java">Thanks to Morten Nobel for implementation hint</a>
     */
     ...
     * <p/>
     * This method is intended to be used to select a specific scaling {@link Method} when a {@link Method#AUTOMATIC} method is specified. This method utilizes the {@link Scale#THRESHOLD_QUALITY_BALANCED} and {@link Scale#THRESHOLD_BALANCED_SPEED} thresholds when selecting which method should be used by comparing the primary dimension (width or height) against the threshold and seeing where the image falls. The primary dimension is determined by looking at the orientation of the image: landscape or square images use their width and portrait-oriented images use their height.
     *
