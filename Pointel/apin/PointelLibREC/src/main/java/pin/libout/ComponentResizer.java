...
public class ComponentResizer extends MouseAdapter {
    ...
    public ComponentResizer(Insets dragInsets) {
        this(dragInsets, new Dimension(10, 10));
    }
     ...
     */
    public ComponentResizer setSnapSize(Dimension snapSize) {
    ...
    protected void changeBounds(Component source, int direction, Rectangle bounds, Point pressed, Point current) {
        ...
        if (EAST == (direction & EAST)) {
            ...
            drag = getDragBounded(drag, snapSize.width, width, minimumSize.width, maximumSize.width);
            width += drag;
        }
        if (SOUTH == (direction & SOUTH)) {
            int drag = getDragDistance(current.y, pressed.y, snapSize.height);
            drag = getDragBounded(drag, snapSize.height, height, minimumSize.height, maximumSize.height);
            height += drag;
        }
        ...
        source.setMinimumSize(dim);
        source.setMaximumSize(dim);
        source.setSize(dim);
        source.validate();
