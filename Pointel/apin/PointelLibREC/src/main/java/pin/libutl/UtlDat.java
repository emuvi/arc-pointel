...
public class UtlDat {
    ...
    public static String formatoSQL = "yyyy-MM-dd";
    public static String formatoMaquina = "yyyy-MM-dd";
    ...
    public static Boolean eh(Object oValor) {
        ...
        if (oValor != null) {
            if (oValor instanceof java.util.Date) {
                retorno = true;
            } else if (oValor instanceof java.sql.Date) {
                retorno = true;
            } else if (oValor instanceof VDat) {
                retorno = true;
            }
        }
        return retorno;
    ...
    public static Boolean eh(Class daClasse) {
        ...
        if (daClasse != null) {
            if (java.util.Date.class.isAssignableFrom(UtlBin.pegaDaPrimitiva(daClasse))) {
                retorno = true;
            } else if (java.sql.Date.class.isAssignableFrom(UtlBin.pegaDaPrimitiva(daClasse))) {
                retorno = true;
            } else if (VDat.class.isAssignableFrom(daClasse)) {
                retorno = true;
            }
        }
        return retorno;
    ...
    public static java.sql.Date pegaSQL(Object doValor) {
        return pegaSQL(UtlTem.pega(doValor, formatoSQL, null));
    }

    public static java.sql.Date pegaSQL(Object doValor, java.util.Date comPadrao) {
        return pegaSQL(UtlTem.pega(doValor, formatoSQL, comPadrao));
    }

    public static java.sql.Date pegaSQL(java.util.Date doTempo) {
        return pegaSQL(doTempo, null);
    }

    public static java.sql.Date pegaSQL(java.util.Date doTempo, java.sql.Date comPadrao) {
        java.sql.Date retorno = comPadrao;
        try {
            retorno = new java.sql.Date(doTempo.getTime());
        } catch (Exception ex) {
        }
        return retorno;
    }
