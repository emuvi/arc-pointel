...
public class UtlHor {
    ...
    public static String formato = "HH:mm:ss";
    public static String formatoSQL = "HH:mm:ss.SSS";
    public static String formatoMaquina = "HH-mm-ss";
    ...
    public static Boolean eh(Object oValor) {
        ...
        if (oValor != null) {
            if (oValor instanceof java.util.Date) {
                retorno = true;
            } else if (oValor instanceof java.sql.Time) {
                retorno = true;
            } else if (oValor instanceof VHor) {
                retorno = true;
            }
        }
        return retorno;
    ...
    public static Boolean eh(Class daClasse) {
        ...
        if (daClasse != null) {
            if (java.util.Date.class.isAssignableFrom(UtlBin.pegaDaPrimitiva(daClasse))) {
                retorno = true;
            } else if (java.sql.Time.class.isAssignableFrom(UtlBin.pegaDaPrimitiva(daClasse))) {
                retorno = true;
            } else if (VHor.class.isAssignableFrom(daClasse)) {
                retorno = true;
            }
        }
        return retorno;
    ...
    public static java.sql.Time pegaSQL(Object doValor) {
        return pegaSQL(UtlTem.pega(doValor, formatoSQL, null));
    }

    public static java.sql.Time pegaSQL(Object doValor, java.util.Date comPadrao) {
        return pegaSQL(UtlTem.pega(doValor, formatoSQL, comPadrao));
    }

    public static java.sql.Time pegaSQL(java.util.Date doTempo) {
        return pegaSQL(doTempo, null);
    }

    public static java.sql.Time pegaSQL(java.util.Date doTempo, java.sql.Time comPadrao) {
        java.sql.Time retorno = comPadrao;
        try {
            retorno = new java.sql.Time(doTempo.getTime());
        } catch (Exception ex) {
        }
        return retorno;
    }
