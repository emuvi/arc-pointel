...
public class RelPedidos extends Relatorio {

  public static String pegaCNPJCPFFormatado(VCrs doCNPJCPF) {
    return UtlCrs.pegaCNPJCPFFormatado(doCNPJCPF.toString());
  }
  ...
  public RelPedidos(Object aCodigo) {
    super("Pedido", Relatorio.Tipo.Mat, RelatorioPagina.padraoMatricial(), new RelatorioCampo[]{
      new RelatorioCampo(1, 1, "PEDIDO       :", RelatorioCampo.Area.INICIO_PAGINA, RelatorioCampo.Tp.TEXTO),
      ...
      new RelatorioCampo(3, 115, "#      PEDIDO      #", RelatorioCampo.Area.INICIO_PAGINA, RelatorioCampo.Tp.TEXTO),
      ...
      new RelatorioCampo(5, 95, "cnpjcpf", RelatorioCampo.Area.INICIO_PAGINA, RelatorioCampo.Tp.CLASSE).botaTamanho(18).botaClasse("pin.adm.RelPedidos", "pegaCNPJCPFFormatado"),
      ...
      new RelatorioCampo(9, 6, "QUANT", RelatorioCampo.Area.INICIO_PAGINA, RelatorioCampo.Tp.TEXTO).botaTamanho(8).botaADireita(),
      ...
      new RelatorioCampo(1, 6, "quantidade", RelatorioCampo.Area.DADOS_DETALHES, RelatorioCampo.Tp.DADO).botaTamanho(8).botaFormato(RelatorioCampo.Formatos.NUMERO_LON, "#0.00").botaADireita(),
      ...
      new RelatorioCampo(1, 100, "pendente", RelatorioCampo.Area.DADOS_DETALHES, RelatorioCampo.Tp.DADO),
      ...
      new RelatorioCampo(2, 111, "ped_desc_tot", RelatorioCampo.Area.FIM_RELATORIO, RelatorioCampo.Tp.DADO).botaTamanho(8).botaFormato(RelatorioCampo.Formatos.NUMERO_LON, "#0.00").botaADireita(),
      new RelatorioCampo(2, 120, "TOTAL:", RelatorioCampo.Area.FIM_RELATORIO, RelatorioCampo.Tp.TEXTO),
      new RelatorioCampo(2, 127, "ped_total_tot", RelatorioCampo.Area.FIM_RELATORIO, RelatorioCampo.Tp.DADO).botaTamanho(8).botaFormato(RelatorioCampo.Formatos.NUMERO_LON, "#0.00").botaADireita(),
      new RelatorioCampo(4, 1, ".", RelatorioCampo.Area.FIM_RELATORIO, RelatorioCampo.Tp.TEXTO)
    });
  ...
  public void produzir() throws Exception {
    ...
    resultados = conexao.busca("SELECT DISTINCT ON (itens_pedido.pedido, itens_pedido.codigo) pedidos.codigo, pedidos.ped_vendedor, "
            ...
            + "  CONCAT('( ', clientes.obs, ' ) ', pedidos.obs1, pedidos.obs2) AS obs, "
            + "  itens_pedido.produto, itens_pedido.quantidade, produtos.unidade, CONCAT(produtos.nome, ' ', itens_pedido.obs) AS item, "
            + "  (CASE WHEN itens_pedido.situacao LIKE 'P' THEN 'PENDENTE' ELSE '' END) AS pendente, "
            + "  itens_pedido.desconto, itens_pedido.desconto_per, itens_pedido.preco, itens_pedido.subtotal, itens_pedido.total, "
            + "  (COALESCE(pedidos.desc_itens, 0) + COALESCE(pedidos.out_desc, 0)) AS ped_desc_tot, pedidos.total AS ped_total_tot "
            + " FROM itens_pedido "
            + " JOIN pedidos ON "
