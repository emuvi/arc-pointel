...
public class RelClientesOciosos extends Relatorio {

    public RelatorioFiltro filtro;

    public RelClientesOciosos() {
        super("Clientes Ociosos", Tipo.Gra, RelatorioPagina.padraoGrafico(), new RelatorioCampo[]{
    ...
    public void produzir() throws Exception {
        filtro = new RelatorioFiltro(this, "pessoas", new RelatorioFiltroCampo[]{
            new RelatorioFiltroCampo(1, 1, "ociosos", "Ociosos Há", Dados.Tp.Int).botaInicial(6).tiraAutomatico(),
            new RelatorioFiltroCampo(1, 2, "ociosos_unid", "Unidade", Dados.Tp.Enu).botaOpcoes(UtlTem.Temporidade.class).botaInicial(UtlTem.Temporidade.Meses).tiraAutomatico(),
            new RelatorioFiltroCampo(2, 1, "nome", "Nome", Dados.Tp.Crs).botaTamanho(80).botaOperador("LIKE '%' || ? || '%'"),
            new RelatorioFiltroCampo(2, 1, "fantasia", "Fantasia", Dados.Tp.Crs).botaTamanho(60).botaOperador("LIKE '%' || ? || '%'"),
            new RelatorioFiltroCampo(3, 1, "grupo", "Grupo - Cód.", Dados.Tp.Crs).botaTamanho(4),
            new RelatorioFiltroCampo(3, 2, "grupos_pessoas.nome", "Grupo - Nome", Dados.Tp.Crs).botaTamanho(60).botaEstrangeiro().tiraOrdenar(),
            new RelatorioFiltroCampo(3, 1, "subgrupo", "SubGrupo - Cód.", Dados.Tp.Crs).botaTamanho(4),
            new RelatorioFiltroCampo(3, 2, "subgrupos_pessoas.nome", "SubGrupo - Nome", Dados.Tp.Crs).botaTamanho(60).botaEstrangeiro().tiraOrdenar(),
            new RelatorioFiltroCampo(4, 1, "cidade", "Cidade - Cód.", Dados.Tp.Crs).botaTamanho(6),
            new RelatorioFiltroCampo(4, 2, "cidades.pais", "Cidade - País", Dados.Tp.Crs).botaTamanho(4).botaEstrangeiro(),
            new RelatorioFiltroCampo(4, 3, "cidades.estado", "Cidade - Estado", Dados.Tp.Crs).botaTamanho(4).botaEstrangeiro(),
            new RelatorioFiltroCampo(4, 4, "cidades.nome", "Cidade - Nome", Dados.Tp.Crs).botaTamanho(60).botaEstrangeiro(),
            new RelatorioFiltroCampo(5, 1, "bairro", "Bairro - Cód.", Dados.Tp.Crs).botaTamanho(4),
            new RelatorioFiltroCampo(5, 2, "bairros.nome", "Bairro - Nome", Dados.Tp.Crs).botaTamanho(60).botaEstrangeiro(),
            new RelatorioFiltroCampo(5, 3, "regiao", "Região - Cód.", Dados.Tp.Crs).botaTamanho(4),
            new RelatorioFiltroCampo(5, 4, "regioes.nome", "Região - Nome", Dados.Tp.Crs).botaTamanho(60).botaEstrangeiro(),
            new RelatorioFiltroCampo(6, 1, "representante", "Representante - Cód.", Dados.Tp.Crs).botaTamanho(8),
            new RelatorioFiltroCampo(6, 2, "representantes.nome", "Representante - Nome", Dados.Tp.Crs).botaTamanho(80).botaEstrangeiro(),});
    ...
    public void finalizar() throws Exception {
        Integer ociosos = filtro.pegaCampo("ociosos").editor.pgVlr().pgInt();
        UtlTem.Temporidade temporidade = (UtlTem.Temporidade) filtro.pegaCampo("ociosos_unid").editor.pgVlr().pg();
        Date anter = UtlTem.percorrido(ociosos, temporidade);
        String santer = "'" + UtlDat.formataMaquina(anter) + "'";
        ...
        cnd += " pessoas.ativo = " + UtlLog.formataSQL(true) + " AND pessoas.cliente = 'S' AND faturas.codigo IS NULL ";
        resultados = conexao.busca("SELECT DISTINCT ON (" + filtro.pegaOrdem("", "pessoas.codigo").replaceAll("DESC", "") + ") "
                ...
                + " LEFT JOIN faturas ON faturas.codigo = (SELECT fat.codigo FROM faturas AS fat WHERE fat.cliente = pessoas.codigo AND fat.emitido_data > " + santer + " LIMIT 1)"
